# System MVP Architecture v0.001
...
%% DOMAIN SERVICES
API --> GIS[GIS Service]
API --> RAIN[Rainfall Service]
API --> HYDRO[Hydrology Service]
API --> HYDRAULIC[Hydraulics Service]
API --> REPORT[Report Service]

%% JOB QUEUE
GIS --> QUEUE[Task Queue]
RAIN --> QUEUE
HYDRO --> QUEUE
HYDRAULIC --> QUEUE

%% WORKERS
QUEUE --> WORKERS[Compute Workers]
